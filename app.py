...
from src.checker import run_checks, run_final_checks, suggest_corrections, apply_corrections
...
from src.t12_checker import run_t12_checks, self_correct_t12
...
# ═══════════════════════════════════════════════════════════════════════════════
# FILE UPLOAD — single section for both files
# ═══════════════════════════════════════════════════════════════════════════════

st.subheader("Upload Files")
st.caption("Upload a Rent Roll, a T12, or both.")

col_rr, col_t12 = st.columns(2)
with col_rr:
    uploaded_file = st.file_uploader("Rent Roll", type=["xlsx", "xls", "csv"])
with col_t12:
    t12_file = st.file_uploader("T12", type=["xlsx", "xls"], key="t12_upload")

if (uploaded_file or t12_file) and not api_key:
    st.warning("Please enter your Anthropic API key in the sidebar.")
    st.stop()

# ─── Build tabs dynamically based on what was uploaded ────────────────────────

has_rr = uploaded_file is not None
has_t12 = t12_file is not None

if has_rr or has_t12:
    client = Anthropic(api_key=api_key)

    tab_names = []
    if has_rr:
        tab_names.append("Rent Roll")
    if has_t12:
        tab_names.append("T12 Analysis")
    ...
    if has_rr:
        with tabs[tab_idx]:
            ...
            if st.session_state.parsed:
                ...
                if st.session_state.check_results is None:
                    with st.spinner("Running data quality checks..."):
                        st.session_state.check_results = run_checks(client, df, as_of_date)
                ...
                if check_results:
                    ...
                    for issue in check_results:
                        ...
                        source = " (AI)" if issue.get("source") == "ai" else ""
                        msg = f"**{issue['check']}**{source}: {issue['detail']}"
                        if sev == "error":
                            st.error(msg)
                        elif sev == "warning":
                            st.warning(msg)
                ...
                if st.session_state.mapping_df is None:
                    ...
                    if st.button("Generate Summary", type="primary"):
                        ...
                        # Final quality checks on aggregated data
                        with st.spinner("Running final quality checks..."):
                            final_issues = run_final_checks(client, agg_data)

                        # Self-correct if errors found
                        if any(i["severity"] == "error" for i in final_issues):
                            with st.spinner("Auto-correcting detected issues..."):
                                corrections = suggest_corrections(client, agg_data, final_issues)
                                if corrections:
                                    agg_data, mapping = apply_corrections(client, agg_data, mapping, corrections)
                                    final_issues = run_final_checks(client, agg_data)

                        st.session_state.agg_data = agg_data
                        st.session_state.final_check_results = final_issues
            ...
            if st.session_state.get("agg_data"):
                ...
                # Final quality checks results
                final_checks = st.session_state.get("final_check_results")
                if final_checks:
                    st.divider()
                    st.subheader("Final Output Checks")
                    for issue in final_checks:
                        sev = issue.get("severity", "info")
                        source_label = ""
                        src = issue.get("source", "")
                        if src == "final_ai":
                            source_label = " (AI)"
                        elif src == "final_check":
                            source_label = ""
                        msg = f"**{issue['check']}**{source_label}: {issue['detail']}"
                        if sev == "error":
                            st.error(msg)
                        elif sev == "warning":
                            st.warning(msg)
                            ...
                            st.info(msg)
                elif st.session_state.get("final_check_results") is not None:
                    st.success("All final output checks passed.")
    ...
    if has_t12:
        with tabs[tab_idx]:
                ...
                if st.session_state.t12_mapping_df is None:
                    if st.button("Map GL Accounts with AI"):
                        with st.spinner("Mapping GL accounts to underwriting categories..."):
                            t12_mapping = propose_t12_mapping(client, t12.leaf_items, t12.grand_totals)
                    ...
                    if st.button("Generate T12 Output", type="primary"):
                        ...
                        # Run T12 checks before output
                        with st.spinner("Running T12 quality checks..."):
                            t12_issues = run_t12_checks(client, t12, t12_mapping)

                        # Self-correct if errors found
                        if any(i["severity"] == "error" for i in t12_issues):
                            with st.spinner("Auto-correcting T12 mapping..."):
                                corrected = self_correct_t12(client, t12, t12_mapping, t12_issues)
                                if corrected:
                                    t12_mapping = corrected
                                    t12_issues = run_t12_checks(client, t12, t12_mapping)

                        st.session_state.t12_check_results = t12_issues
            ...
            # T12 check results
            t12_checks = st.session_state.get("t12_check_results")
            if t12_checks:
                st.divider()
                st.subheader("T12 Quality Checks")
                for issue in t12_checks:
                    sev = issue.get("severity", "info")
                    source_label = " (AI)" if issue.get("source") == "t12_ai" else ""
                    msg = f"**{issue['check']}**{source_label}: {issue['detail']}"
                    if sev == "error":
                        st.error(msg)
                    elif sev == "warning":
                        ...
                        st.info(msg)
            elif st.session_state.get("t12_check_results") is not None:
                st.success("All T12 quality checks passed.")
